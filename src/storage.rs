...
pub async fn save_new_fw<S: NorFlash, R: Read>(
    ...
    storage.erase(inactive_partition.offset, inactive_partition.offset + inactive_partition.size as u32).map_err(|_| UpgradeError::StorageError)?;
    ...
    while !done_reading {
        ...
        while amount_read < SECTOR_SIZE {
            let size = binary_reader.read(&mut write_buffer[amount_read..]).await.map_err(|_| UpgradeError::StorageError)?;
            if size == 0  {
                done_reading = true;
            }
            amount_read += size;
        ...
        storage.write(inactive_partition.offset + amount_read as u32,
...
fn accept_fw<S: NorFlash>(storage: &mut S) -> Result<()> {
